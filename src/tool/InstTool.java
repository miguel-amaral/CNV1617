...
public class InstTool{
	...
	public static void main(String argv[]){
			...
			logger = new Logger("myfile");

			if (argv.length < 2 || !argv[0].startsWith("-")) {
				//printUsage("< 2 || !argv[0].startsWith(-) ");
				logger.writeLine("< 2 || !argv[0].startsWith(-) ");
			}

			if (argv[0].equals("-static")) {
				if (argv.length != 2) {
					printUsage("argv[0].equals(-static) && argv.length != 2 ");
				}
				...
				try {
					...
					if (in_file.isFile()) {
						...
						printUsage("argv[0].equals(-static) && argv.length != 2 && !in_dir.isDirectory");
					}
				}
				catch (NullPointerException e) {
					printUsage("argv[0].equals(-static) && argv.length != 2 && NullPointerException");
				}
			}
				...
				try {
					...
					if (in_file.isFile() && out_dir.isDirectory()) {
        ...
        for (Enumeration e = ci.getRoutines().elements(); e.hasMoreElements(); ) {
          ...
          for (Enumeration instrs = instructions.elements(); instrs.hasMoreElements(); ) {
            ...
            if ((opcode==InstructionTable.NEW) ||
              ...
              instr.addBefore("InstTool", "allocCount", new Integer(opcode));
            }
          }
        }
        ci.addAfter("InstTool", "printAlloc", "null");
        ci.write(out_filename);
      }
    }
  }
        ...
        for (Enumeration e = ci.getRoutines().elements(); e.hasMoreElements(); ) {
          ...
          for (Enumeration instrs = (routine.getInstructionArray()).elements(); instrs.hasMoreElements(); ) {
            ...
            if (opcode == InstructionTable.getfield)
              instr.addBefore("InstTool", "LSFieldCount", new Integer(0));
            else if (opcode == InstructionTable.putfield)
              instr.addBefore("InstTool", "LSFieldCount", new Integer(1));
            else {
              short instr_type = InstructionTable.InstructionTypeTable[opcode];
              if (instr_type == InstructionTable.LOAD_INSTRUCTION) {
                instr.addBefore("InstTool", "LSCount", new Integer(0));
              }
              else if (instr_type == InstructionTable.STORE_INSTRUCTION) {
                instr.addBefore("InstTool", "LSCount", new Integer(1));
              }
            }
          }
        }
        ci.addAfter("InstTool", "printLoadStore", "null");
        ci.write(out_filename);
      }
    }
  }
